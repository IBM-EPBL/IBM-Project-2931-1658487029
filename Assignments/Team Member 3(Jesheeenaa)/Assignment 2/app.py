...
app.config['MYSQL_PASSWORD'] = 'jesheenaa'
...
# Output message if something goes wrong...
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        account = cursor.fetchone()
                # If account exists in user_data table in out database
        if account:
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        cursor.execute( "SELECT * FROM user_data WHERE username LIKE %s", [username] )
        account = cursor.fetchone()
        ...
        # Account doesnt exists and the form data is valid, now insert new account into user_data table
            cursor.execute('INSERT INTO user_data VALUES (%s, %s, %s)', (username,email, password))
